...
const ThermalMap = forwardRef<ThermalMapRef, ThermalMapProps>(
	...
	useEffect(() => {
		...
		const handleMouseMove = (e: MapMouseEvent) => {
			...
			if (features && features.length > 0) {
				...
				popup
					.setLngLat(e.lngLat)
					.setHTML(`<div class="p-2"><strong>Temperatura:</strong> ${temperature.toFixed(1)}°C</div>`)
					.addTo(map.current!);
			} else {
				popup.remove();
			}
	...
	useEffect(() => {
		...
		// Add routes layer
		const routeHandlers: Array<{ routeId: string; handler: (e: MapMouseEvent) => void }> = [];
			...
			{!mapLoaded && (
				<div className="absolute inset-0 flex items-center justify-center bg-gray-100">
					<div className="text-center">
						...
						<div className="text-sm text-muted-foreground">Aguarde enquanto inicializamos o mapa</div>
					</div>
				</div>
			)}
		</div>
